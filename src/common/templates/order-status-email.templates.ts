/**
 * Email templates for order status notifications sent to customers.
 */

const baseStyles = {
  wrapper:
    'margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background-color: #000000; color: #ffffff;',
  outerTable: 'background: radial-gradient(circle at top, #f5f5f5, #000000 55%); padding: 24px 0;',
  container:
    'background-color: #000000; border-radius: 18px; overflow: hidden; max-width: 640px; width: 100%; border: 1px solid #444444;',
  header: 'padding: 20px 26px 16px;',
  content: 'padding: 24px 26px 26px;',
  footer:
    'background-color: #000000; padding: 16px 26px 20px; text-align: center; border-top: 1px solid #444444;',
};

export function generateOrderPlacedEmail(
  customerName: string,
  orderId: number,
  totalAmount: number,
  productList: string,
...
export function generateOrderProcessingEmail(
  customerName: string,
  orderId: number,
  storeName: string,
  trackingUrl?: string,
